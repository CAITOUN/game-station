...
// localStorage键名
const GAME_ANALYTICS_KEY = 'game_analytics';
...
// 获取个性化推荐游戏ID列表
export function getPersonalizedGameIds(count: number = 10): number[] {
  const userHistory = getUserHistory();
  
  if (userHistory.recentlyPlayed.length === 0) {
    // 如果没有游戏历史，返回热门游戏
    return getPopularGameIds(count);
  }
  
  // 基于用户偏好分类的推荐
  const preferredTags = userHistory.favoriteCategories.slice(0, 5); // 取前5个偏好分类
  
  return userHistory.recentlyPlayed.slice(0, count);
}
...
export function cleanupOldData(): void {
  ...
  try {
    ...
    // 清理30天前的访问历史
    userHistory.visitHistory = userHistory.visitHistory.filter(
      visit => visit.lastVisited > thirtyDaysAgo
    );
    ...
    // 清理全局统计中的过期数据
    const stats = getGlobalStats();
    const cleanedStats = stats.filter(stat => stat.lastVisited > thirtyDaysAgo);
    localStorage.setItem(GLOBAL_STATS_KEY, JSON.stringify(cleanedStats));
  } catch (error) {
    console.warn('Failed to cleanup old data:', error);
  }
}

// 重新导出games.ts中的函数以便客户端组件使用
export { getRecommendedForYou, getWeeklyPopularGames } from './games'; 